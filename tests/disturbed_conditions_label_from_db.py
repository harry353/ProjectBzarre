    ...
    with sqlite3.connect(LABELS_DB) as conn:
        ...
        for table in (TRAIN_TABLE, VAL_TABLE, TEST_TABLE):
            frame = pd.read_sql_query(
                f"SELECT time_tag, disturbed_flag FROM {table}",
                conn,
                parse_dates=["time_tag"],
            )
            frames.append(frame)
    labels = pd.concat(frames, ignore_index=True)
    labels["time_tag"] = pd.to_datetime(labels["time_tag"], errors="coerce", utc=True)
    labels = labels.dropna(subset=["time_tag"]).set_index("time_tag").sort_index()
    return labels["disturbed_flag"].astype(bool)
...
def _load_aligned_data() -> pd.DataFrame:
    ...
    labels = _load_labels().to_frame("disturbed_flag")
    return imf.join([dst, kp, labels], how="inner")
...
def plot_disturbed_conditions() -> None:
    ...
    disturbed_flag = df["disturbed_flag"].fillna(False)
    ...
    peak_indices, _ = find_peaks(kp_values, height=5.0)
