...
OUTPUT_DB = STAGE_DIR / "imf_solar_wind_aver_comb_filt_imp_eng_split.db"
...
def create_imf_solar_wind_splits() -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    if df.empty:
        raise RuntimeError("IMF + solar wind feature dataset is empty; run feature engineering first.")
    ...
    print(f"[OK] IMF + solar wind train/val/test splits stored at {OUTPUT_DB}")
