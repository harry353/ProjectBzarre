...
FEATURES_DB = (
    STAGE_DIR.parents[1] / "dst" / "6_aggregate" / "dst_agg_eng.db"
)
FEATURES_TABLE = "features_agg"
OUTPUT_DB = STAGE_DIR / "dst_agg_eng_split.db"
...
def _prepare_index(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if not isinstance(df.index, pd.DatetimeIndex):
        raise ValueError("Aggregated DST dataset must have a DatetimeIndex.")
...
def create_dst_splits() -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    if df.empty:
        raise RuntimeError("DST aggregated dataset is empty; run aggregate step first.")
    ...
    print(f"[OK] DST aggregate train/val/test splits stored at {OUTPUT_DB}")
