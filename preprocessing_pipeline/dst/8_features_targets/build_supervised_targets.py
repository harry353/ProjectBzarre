...
NORMALIZED_DB = STAGE_DIR.parents[1] / "dst" / "7_normalization" / "dst_aver_filt_imp_eng_split_norm.db"
...
def _build_supervised(df: pd.DataFrame, predictors: Sequence[str], horizon: int) -> pd.DataFrame:
    ...
    for candidate in ("timestamp", "time_tag"):
        if candidate in df.columns:
            ...
            break
    if time_col:
        timestamp_values = pd.to_datetime(df.loc[mask, time_col], utc=True, errors="coerce")
        features.insert(0, "timestamp", timestamp_values.values)
...
def build_dst_supervised(horizon: int) -> None:
    ...
    output_db = STAGE_DIR / f"dst_aver_filt_imp_eng_split_norm_h{horizon}.db"
    ...
    if horizon == 6:
        destination = STAGE_DIR.parents[1] / "dst" / "dst_fin.db"
        destination.write_bytes(output_db.read_bytes())
        print(f"[OK] Copied {output_db.name} to {destination}")
