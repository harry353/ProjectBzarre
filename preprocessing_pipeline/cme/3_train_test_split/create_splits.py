...
OUTPUT_DB = STAGE_DIR / "cme_agg_eng_split.db"
...
def _load_daily() -> pd.DataFrame:
    ...
    raise RuntimeError("CME aggregate dataset missing timestamp/date/time_tag column.")


def create_cme_splits() -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    df = _load_daily()
    if df.empty:
        raise RuntimeError("CME daily feature dataset is empty; run aggregation first.")
    ...
    print(f"[OK] CME train/val/test splits stored at {OUTPUT_DB}")
