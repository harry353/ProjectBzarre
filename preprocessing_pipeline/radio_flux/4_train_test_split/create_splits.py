...
FEATURES_DB = (
    STAGE_DIR.parents[1] / "radio_flux" / "3_aggregate" / "radio_flux_agg_eng.db"
)
FEATURES_TABLE = "features_agg"
OUTPUT_DB = STAGE_DIR / "radio_flux_agg_eng_split.db"
...
def create_radio_flux_splits() -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    if "timestamp" in df.columns:
        ...
        raise RuntimeError("Radio flux aggregate dataset missing timestamp/date column.")
    if df.empty:
        raise RuntimeError("Radio flux aggregate dataset is empty; run aggregation first.")
    ...
    print(f"[OK] Radio flux train/val/test splits stored at {OUTPUT_DB}")
    return train, val, test


def main() -> None:
    create_radio_flux_splits()
