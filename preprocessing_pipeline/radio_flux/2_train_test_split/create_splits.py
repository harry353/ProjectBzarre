...
FEATURES_TABLE = "filtered_data"
OUTPUT_DB = STAGE_DIR / "radio_flux_filt_split.db"
...
def create_radio_flux_splits() -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    if "timestamp" in df.columns:
        ...
        raise RuntimeError("Radio flux filtered dataset missing timestamp/date column.")
    if df.empty:
    ...
    print(f"[OK] Radio flux filtered train/val/test splits stored at {OUTPUT_DB}")
