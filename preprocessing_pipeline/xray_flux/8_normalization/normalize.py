...
# ---------------------------------------------------------------------
# Column groupings (AUTHORITATIVE)
# ---------------------------------------------------------------------
Z_COLS: Set[str] = {
    # Core log-fluxes
    "log_xrsb",
    ...
    # Lagged and delta features from engineering
    "log_xrsb_lag_1h",
    "log_xrsa_lag_1h",
    "dlog_xrsb_1h",
    "dlog_xrsa_1h",
...
LOG_Z_COLS: Set[str] = {
    # Flare contrast
    "log_xrsb_peak_to_bg_24h",
    "log_xrsa_peak_to_bg_24h",
}

LOG1P_Z_COLS: Set[str] = {
    # Event recency
    "hrs_since_rapid_rise_xrsb_24h",
    "hrs_since_rapid_rise_xrsa_24h",
}
...
def _fit_params(train: pd.DataFrame) -> Dict[str, Dict[str, float]]:
    ...
    missing = numeric_cols - classified
    if missing:
        raise RuntimeError(
            f"Unclassified numeric X-ray columns: {sorted(missing)}"
        )
...
def _apply_normalization(
    ...
    for col, info in params.items():
        if info["method"] == "z":
            out[col] = (_as_numeric(out[col]) - info["mean"]) / info["std"]
...
def _load_params() -> Dict[str, Dict[str, float]]:
    if not PARAMS_PATH.exists():
        raise RuntimeError(
            f"Missing normalization parameters: {PARAMS_PATH}"
        )
...
def _normalize_inference() -> None:
    ...
    print(f"[OK] Applied saved normalization to inference dataset")
