...
SPLITS_DB = (
    ...
    / "xray_flux"
    / "6_train_test_split"
    / "xray_flux_aver_filt_imp_eng_split.db"
)
...
OUTPUT_DB = STAGE_DIR / "xray_flux_aver_filt_imp_eng_split_norm.db"
FINAL_COPY = STAGE_DIR.parents[1] / "xray_flux" / "xray_flux_fin.db"
...
def apply_normalization(df: pd.DataFrame, params: dict[str, dict]) -> pd.DataFrame:
    ...
    if norm_cols:
        if out[norm_cols].isna().any().any():
            raise RuntimeError("NaNs introduced during X-ray normalization.")
...
# Main pipeline
# ---------------------------------------------------------------------
def normalize_xray_splits() -> dict[str, pd.DataFrame]:
    train = _load_split(TRAIN_TABLE)
    val = _load_split(VAL_TABLE)
    test = _load_split(TEST_TABLE)

    if train.empty or val.empty or test.empty:
        raise RuntimeError("One or more X-ray data splits are empty.")
    ...
    with sqlite3.connect(OUTPUT_DB) as conn:
        norm_train.to_sql(
            TRAIN_TABLE, conn, if_exists="replace", index_label="time_tag"
        )
        norm_val.to_sql(
            VAL_TABLE, conn, if_exists="replace", index_label="time_tag"
        )
        norm_test.to_sql(
            TEST_TABLE, conn, if_exists="replace", index_label="time_tag"
        )

    PARAMS_PATH.write_text(json.dumps(params, indent=2))
    FINAL_COPY.write_bytes(OUTPUT_DB.read_bytes())

    print(f"[OK] Normalized X-ray splits written to {OUTPUT_DB}")
