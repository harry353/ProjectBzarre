...
OUTPUT_DB = STAGE_DIR / "xray_flux_agg_eng_split.db"
...
def _prepare_index(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if not isinstance(df.index, pd.DatetimeIndex):
        raise ValueError("X-ray aggregate dataset must have a DatetimeIndex.")
...
def create_xray_flux_splits() -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    if df.empty:
        raise RuntimeError("X-ray aggregate dataset is empty; run aggregate stage first.")
    ...
    if train.empty or val.empty or test.empty:
        raise RuntimeError("Unable to create non-empty X-ray daily splits; adjust PREPROC_SPLIT_* windows.")
    ...
    print(f"[OK] X-ray aggregate train/val/test splits stored at {OUTPUT_DB}")
    return train, val, test


def main() -> None:
    create_xray_flux_splits()
