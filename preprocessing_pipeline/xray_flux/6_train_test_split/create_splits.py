...
OUTPUT_DB = STAGE_DIR / "xray_flux_aver_filt_imp_eng_split.db"
...
def create_xray_flux_splits() -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    if df.empty:
        raise RuntimeError("X-ray flux feature dataset is empty; run feature engineering first.")
    ...
    if train.empty or val.empty or test.empty:
        raise RuntimeError("Unable to create non-empty X-ray flux splits; adjust PREPROC_SPLIT_* windows.")
    ...
    print(f"[OK] X-ray flux train/val/test splits stored at {OUTPUT_DB}")
