...
OUTPUT_DB = STAGE_DIR / "xray_flux_imputed_split.db"
...
def _prepare_index(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if not isinstance(df.index, pd.DatetimeIndex):
        raise ValueError("X-ray imputed dataset must have a DatetimeIndex.")
...
def create_xray_flux_splits() -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    if df.empty:
        raise RuntimeError("X-ray imputed dataset is empty; run imputation stage first.")
    ...
    print(f"[OK] X-ray imputed train/val/test splits stored at {OUTPUT_DB}")
