...
OUTPUT_DB = STAGE_DIR / "kp_imputed_split.db"
...
def _prepare_index(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if not isinstance(df.index, pd.DatetimeIndex):
        raise ValueError("KP imputed dataset must have a DatetimeIndex.")
...
def create_kp_splits() -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    if df.empty:
        raise RuntimeError("KP imputed dataset is empty; run imputation step first.")
    ...
    print(f"[OK] KP imputed train/val/test splits stored at {OUTPUT_DB}")
