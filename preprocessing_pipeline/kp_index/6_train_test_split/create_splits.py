...
OUTPUT_DB = STAGE_DIR / "kp_index_aver_filt_imp_eng_split.db"
...
def create_kp_splits() -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    if df.empty:
        raise RuntimeError("KP features dataset is empty; run feature engineering first.")
    ...
    print(f"[OK] KP train/val/test splits stored at {OUTPUT_DB}")
