...
def _build_daily_targets(series: pd.Series) -> pd.DataFrame:
    ...
    rows = []
    chunk = pd.Timedelta(hours=8)
    current = start_day
    last_possible = series.index.max().floor("H") - chunk
    while current <= last_possible:
        window_end = current + chunk
        mask = (series.index >= current) & (series.index < window_end)
        severity = int(series[mask].max()) if mask.any() else 0
        rows.append({"date": current, "storm_severity_next_8h": severity})
        current = window_end
    if not rows:
        raise RuntimeError("8h label generation produced no rows.")
