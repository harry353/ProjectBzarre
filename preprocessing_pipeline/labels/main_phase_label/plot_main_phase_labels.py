...
SSC_DB = STAGE_DIR / "main_phase_labels.db"
SSC_TABLES = [
    "storm_main_phase_train",
    "storm_main_phase_validation",
    "storm_main_phase_test",
]
...
def _load_main_phase_labels() -> pd.DataFrame:
    frames = []
...
def plot_kp_year(year: int, output: Path | None = None) -> None:
    dst = _load_dst()
    kp = _load_kp()
    main_phase_labels = _load_main_phase_labels()
    ...
    main_phase_slice = main_phase_labels.loc[
        (main_phase_labels["timestamp"] >= start) & (main_phase_labels["timestamp"] < end)
    ]

    if dst_slice.empty or kp_slice.empty:
        raise ValueError(f"No overlapping DST/Kp data for year {year}.")
    if "storm_flag" not in main_phase_slice.columns or "storm_severity" not in main_phase_slice.columns:
    ...
    spans = _storm_spans(main_phase_slice)
