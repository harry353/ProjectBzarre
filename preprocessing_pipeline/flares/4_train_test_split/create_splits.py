...
OUTPUT_DB = STAGE_DIR / "flare_comb_filt_eng_split.db"
...
def create_flare_splits() -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    if df.empty:
        raise RuntimeError("Flare feature dataset is empty; run feature engineering first.")
    ...
    print(f"[OK] Flare train/val/test splits stored at {OUTPUT_DB}")
