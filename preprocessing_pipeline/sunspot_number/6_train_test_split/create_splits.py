...
OUTPUT_DB = STAGE_DIR / "sunspot_number_aver_filt_imp_eng_split.db"
...
def create_sunspot_splits() -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    if df.empty:
        raise RuntimeError("Sunspot feature dataset is empty; run feature engineering first.")
    ...
    print(f"[OK] Sunspot train/val/test splits stored at {OUTPUT_DB}")
