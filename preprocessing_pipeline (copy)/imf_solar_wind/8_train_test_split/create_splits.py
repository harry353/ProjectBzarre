...
FEATURES_DB = (
    ...
    / "imf_solar_wind"
    / "7_aggregate"
    / "imf_solar_wind_agg_eng.db"
)
FEATURES_TABLE = "features_agg"
OUTPUT_DB = STAGE_DIR / "imf_solar_wind_agg_eng_split.db"
...
def _prepare_index(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if not isinstance(df.index, pd.DatetimeIndex):
        raise ValueError("Aggregated IMF + solar wind dataset must have a DatetimeIndex.")
...
def create_imf_solar_wind_splits() -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    if df.empty:
        raise RuntimeError("IMF + solar wind aggregate dataset is empty; run aggregate step first.")
    ...
    print(f"[OK] IMF + solar wind aggregate train/val/test splits stored at {OUTPUT_DB}")
