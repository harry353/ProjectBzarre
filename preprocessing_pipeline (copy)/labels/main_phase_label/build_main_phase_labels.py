...
TRAIN_TABLE = "storm_main_phase_train"
VAL_TABLE = "storm_main_phase_validation"
TEST_TABLE = "storm_main_phase_test"
...
def _compute_ranges(
    ...
    for peak_time in peaks:
        ...
        storm_window = dst_series[(dst_series.index >= start) & (dst_series.index <= end)]
        if storm_window.empty:
            continue
        min_time = storm_window.idxmin()
        if min_time <= start:
            continue
        end = min_time
...
def build_main_phase_labels() -> None:
    dst = _load_dst()
    kp = _load_kp()
    ap = _kp_to_ap(kp)
