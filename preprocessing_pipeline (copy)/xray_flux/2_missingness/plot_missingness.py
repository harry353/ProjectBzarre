...
FIGURE_PATH = STAGE_DIR / "xray_flux_missingness.png"
COLUMNS = [
    "irradiance_xrsa1",
    "irradiance_xrsa2",
    "irradiance_xrsb1",
...
def plot_xray_missingness() -> None:
    df = load_hourly_output(HOURLY_DB, HOURLY_TABLE)
    if df.empty:
        raise RuntimeError("X-ray hourly dataset is empty; run averaging first.")
    ...
    ax.set_title("X-ray flux time between valid samples")
