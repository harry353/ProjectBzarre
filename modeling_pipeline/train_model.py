...
from modeling_pipeline.feature_pruning import select_pruned_features
...
TARGET_HORIZONS = range(1, 9)

MIN_PRUNED_FEATURES = 50
...
def _train_and_evaluate(
    ...
    feature_cols = feature_columns(splits["train"], target_col)
    ...
    train_df = train_df_full[["forecast_date", target_col] + feature_cols].copy()
    val_df = val_df_full[["forecast_date", target_col] + feature_cols].copy()
    test_df = test_df_full[["forecast_date", target_col] + feature_cols].copy()
...
def main() -> None:
    for horizon in TARGET_HORIZONS:
        ...
        print(f"[INFO] Training horizon h{horizon} -> {output_dir}")
        splits = load_split_tables(MERGED_DB, horizon)
