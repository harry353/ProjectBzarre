...
# Launch the log-zscore transformation script as a subprocess to normalize live inference data
def _run_log_zscore() -> None:
    ...
    # The command applies the training-time normalization parameters to the 6m inference features
    cmd = [
        sys.executable,
        str(LOG_Z_SCRIPT),
        "--db",
...
# Utility to load a SQLite table into a pandas DataFrame
def _load_table(db_path: Path, table: str) -> pd.DataFrame:
    with sqlite3.connect(db_path) as conn:
        return pd.read_sql_query(f"SELECT * FROM {table}", conn)


# Apply the pre-trained PCA model to the normalized inference data
def _apply_saved_pca() -> None:
    ...
    # Load PCA model and feature metadata (which columns were used, which were passthrough)
    ...
    # Load the normalized table (all splits point to the same 6m snapshot during inference)
    table_name = "transformed_inf_train"
    df = _load_table(PCA_OUT_DB, table_name)
    if df.empty:
        raise RuntimeError(f"Input table '{table_name}' is empty.")
    
    # Validate that all required PCA features exist in the current data stream
    ...
    # Re-attach temporal metadata (timestamp/time_tag) to the new PCA features
    ...
    with sqlite3.connect(PCA_OUT_DB) as conn:
        # Save the final regression-ready vector and clean up intermediate normalization tables
...
# Main sequence: Normalize -> Apply PCA
def main() -> None:
    _run_log_zscore()
    _apply_saved_pca()
