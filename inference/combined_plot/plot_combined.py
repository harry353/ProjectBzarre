...
from inference.combined_plot.constants import (
    PROJECT_ROOT,
    DST_DB,
    DST_TABLE,
    FEATURE_DB,
...
from inference.combined_plot.plot_dst_main import plot_dst_main
from inference.combined_plot.plot_prob_main import plot_prob_main
from inference.combined_plot.plot_dst_zoom import plot_dst_zoom
...
def main() -> None:
    ...
    if not ts_col_dst or not dst_col:
        raise RuntimeError("DST table must contain a timestamp column and a numeric target.")
    ...
    for reg_name, data in history_preds.items():
        ...
        q50_sorted = np.array(data["q50"])[sort_idx]
    ...
    if hist_segments and forecast_times:
        ...
        if _as_ts(last_seg["end"]) < first_future_ts:
            bridge_end = first_future_ts - pd.Timedelta(minutes=1)
            if bridge_end < _as_ts(last_seg["end"]):
                bridge_end = _as_ts(last_seg["end"])
            ts_bridge = pd.to_datetime(
                np.linspace(_as_ts(last_seg["end"]).value, bridge_end.value, 3)
            )
            hist_segments.append(
                {
                    "regime": regime,
                    "color": color,
                    "ts": ts_bridge,
                    "q10": np.linspace(last_seg["q10"][-1], q10_list[0], len(ts_bridge)),
                    "q50": np.linspace(last_seg.get("q50", last_seg["q10"][-1]), q50_list[0], len(ts_bridge)),
                    "q90": np.linspace(last_seg["q90"][-1], q90_list[0], len(ts_bridge)),
                    "start": ts_bridge[0],
                    "end": ts_bridge[-1],
                    "interp": True,
                }
    ...
    plot_prob_main(ax_prob, prob_df, HISTORY_HOURS)
    ...
    if not dst_hist.empty:
        x_min_candidates.append(_as_ts(dst_hist[ts_col_dst].min()))
        x_max_candidates.append(_as_ts(dst_hist[ts_col_dst].max()))
