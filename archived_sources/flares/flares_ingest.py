"""SQLite ingestion for GOES flare realtime/archive rows."""
...
def ingest_flares(df, warehouse, dataset: str):
    if df.empty:
        return 0

    dataset = (dataset or "realtime").lower()
    if dataset == "archive":
        return _ingest_archive(df, warehouse)
    return _ingest_realtime(df, warehouse)


def _ingest_realtime(df, warehouse):
    warehouse.ensure_table(REALTIME_TABLE_SQL)
    payload = []
    ...
    return warehouse.insert_rows(REALTIME_INSERT_SQL, payload)


def _ingest_archive(df, warehouse):
    warehouse.ensure_table(ARCHIVE_TABLE_SQL)
    payload = []
    for _, row in df.iterrows():
        payload.append(
            [
    ...
    return warehouse.insert_rows(ARCHIVE_INSERT_SQL, payload)
