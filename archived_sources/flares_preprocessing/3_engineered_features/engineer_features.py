...
def engineer_flare_features() -> pd.DataFrame:
    ...
    hourly["flare_count_last_6h"] = _rolling_sum_np(hourly_counts, 6)
    hourly["flare_count_last_24h"] = _rolling_sum_np(hourly_counts, 24)
    hourly["flare_count_last_72h"] = _rolling_sum_np(hourly_counts, 72)
    ...
    if hourly.isna().any().any():
        ...
        raise RuntimeError(f"NaNs detected in flare feature table: {details}")
