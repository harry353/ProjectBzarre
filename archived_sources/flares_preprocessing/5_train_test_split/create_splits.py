...
OUTPUT_DB = STAGE_DIR / "flare_agg_eng_split.db"
...
def create_flare_splits() -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    if "date" not in df.columns:
        raise RuntimeError("Daily flare dataset missing 'date' column.")
    df = df.set_index("date")
    if df.empty:
        raise RuntimeError("Flare aggregate dataset is empty; run aggregation first.")
    ...
    if train.empty or val.empty or test.empty:
        raise RuntimeError("Unable to create non-empty temporal flare splits; adjust PREPROC_SPLIT_* windows.")
    ...
    print(f"[OK] Flare aggregate train/val/test splits stored at {OUTPUT_DB}")
