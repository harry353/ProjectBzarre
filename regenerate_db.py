...
def process_source_week(
    ...
    if boundary is not None and boundary.date() >= week_end:
        print(
            f"[SKIP] {class_name} already processed through {boundary.date().isoformat()}"
        )
        return

    window_label = f"{week_start.isoformat()} -> {week_end.isoformat()}"
    print(f"Processing {class_name} for {window_label}...")

    try:
        source = cls(**build_source_kwargs(cls, week_start, week_end))
        df = source.download()
    except Exception as exc:
        print(f"[ERROR] {class_name} download failed for {window_label}: {exc}")
        return

    if df is None or df.empty:
        print(f"[INFO] No data for {class_name} during {window_label}. Continuing.")
        return

    try:
        inserted = source.ingest(df, warehouse=warehouse)
    except Exception as exc:
        print(f"[ERROR] {class_name} ingest failed for {window_label}: {exc}")
        return

    if inserted:
        print(f"[OK] Inserted {inserted} rows for {window_label}.")
        _record_latest_timestamp(class_name, df, tracker, class_names)
    else:
        print(
            f"[INFO] {class_name} returned data but nothing was stored for {window_label}."
        )
...
def main() -> None:
    ...
    for index, (week_start, week_end) in enumerate(week_ranges, start=1):
        ...
        for cls in classes:
            process_source_week(cls, warehouse, week_start, week_end, tracker, class_names)
