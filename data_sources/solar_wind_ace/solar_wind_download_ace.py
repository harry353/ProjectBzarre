...
# CDAWeb base URLs for the two ACE/SWEPAM product levels
...
# From this date onward, the K1 (key-parameter) product is used instead of H0
...
def download_solar_wind_ace(start_date: date, end_date: date) -> pd.DataFrame:
    ...
    # Log any consecutive missing day ranges as a single warning
    ...
    # Trim to exactly the requested range (CDF files may contain extra boundary rows)
...
def _fetch_day(day: date) -> tuple[Optional[pd.DataFrame], bool]:
    # Resolve the exact versioned CDF filename from the directory listing
    ...
    # Write to a temp file because cdflib requires a filesystem path
        ...
        df = pd.DataFrame(
            {
                ...
                # Vp: proton bulk speed (km/s)
                "speed": _clean_variable(cdf, "Vp"),
                # Tpr: proton radial temperature (K)
...
def _emit_missing_ranges(label: str, days: list[date]) -> None:
    ...
    # Collapse consecutive missing days into contiguous ranges for cleaner output
    ranges = []
    start = prev = days[0]
    for day in days[1:]:
        if (day - prev).days == 1:
...
def _clean_variable(cdf: cdflib.CDF, variable: str) -> np.ndarray:
    ...
    # Replace the CDF fill sentinel with NaN so downstream math is unaffected
...
def _resolve_filename(day: date) -> Optional[str]:
    # Select product level based on the switch date
    ...
    # Fetch the HTML directory listing to discover the versioned filename
    response = http_get(
        directory,
        timeout=60,
        log_name="Solar Wind ACE",
    ...
    # Match the highest available version for the requested day
