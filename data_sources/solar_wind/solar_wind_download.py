...
def download_solar_wind(start_date, end_date, max_workers=8):
    """
    ...
    if not days:
        return pd.DataFrame(columns=SW_COLUMNS)
    ...
    def process_day(day):
        ...
        response = http_get(directory, log_name="Solar Wind", timeout=15)
        if response is None:
            return None
        html = response.text
        ...
        if not matches:
            print(f"[INFO] No F1M match for {format_date(day)}")
            return None

        filename = matches[0]
        file_url = directory + filename

        response = http_get(file_url, log_name="Solar Wind", timeout=20)
        if response is None:
            return None
