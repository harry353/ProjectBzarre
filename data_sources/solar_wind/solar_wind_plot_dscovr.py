...
def plot_solar_wind_dscovr(df: pd.DataFrame) -> None:
    _plot_common(df, title="DSCOVR Proton Parameters")


def _plot_common(df: pd.DataFrame, title: str) -> None:
    if df.empty:
        print("[WARN] DSCOVR dataframe empty. Cannot plot.")
        return
    ...
    axes[0].plot(speed.index, speed.values, color="tab:green")
    ...
    axes[2].set_title(f"{title} – Temperature")
